//fixed the a issue.... in task 1_2 the user was able to put 1e2 as input and the program was not able to handle it so i fixed it by adding cin.peek() != '\n' in the if condition of getUserGuess function
#include <iostream>
#include <cstdlib>
#include <ctime>
#include <string>
...
public:
    ...
    int getUserGuess(int low, int high) { // this function is for getting the user input and checking if the input is in the given range or not...
      ...
      if (cin.fail() || cin.peek() != '\n' || guess < low || guess > high) { // checks if input is in range or not....
        Invalid_value_checker(); 
...
int main() {
    srand(static_cast<unsigned int>(time(0))); //random number are  generated here
    GuessingGame game;
    game.menu();
    return 0;
}
